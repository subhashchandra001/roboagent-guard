...
def live_check(base_url: str) -> int:
    ...
    with httpx.Client(timeout=75.0) as client:
        ...
        assert client.get(f"{base}/SKILL.md").status_code == 200
        assert client.get(f"{base}/skill.md").status_code == 200
        ...
        safe_response = client.post(f"{base}/v1/evaluate", json=safe)
        safe_response.raise_for_status()
        unsafe_response = client.post(f"{base}/v1/evaluate", json=unsafe)
        ...
        scenario = client.post(f"{base}/v1/scenarios/normal_navigation/run?seed=42")
